...
async function loadImageMap() {
    const cached = localStorage.getItem("imageMapCache");
    if (cached) {
        imageMap = JSON.parse(cached);
        updateGrid();
        console.log("✅ 使用本地缓存的 imageMap");
        return;
    }
    ...
    try {
        ...
        // ✅ 存入缓存，供下次使用
        localStorage.setItem("imageMapCache", JSON.stringify(imageMap));

        updateGrid();
        console.log("✅ 从服务器加载并缓存 imageMap");
    } catch (err) {
...
function findMatchingImages(char) {
    const indices = imageMap[char];
    if (!indices || indices.length === 0) return null;
    return indices.map(index => getImageFilename(char, index));
}
